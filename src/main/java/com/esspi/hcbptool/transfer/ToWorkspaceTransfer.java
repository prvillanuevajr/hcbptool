 ...
 */
public class ToWorkspaceTransfer extends Transfer{
    
    @Override
    public List<Future> transfer(){
        List<Future> futures = new ArrayList<>();
        for (String folder : ToolConfig.getInstance().getSelectedFolders()) {
            Path repoFolderPath = Paths.get(ToolConfig.getInstance().getRepoPath().toString(), folder);
            File destDir = new File(ToolConfig.getInstance().getWorkspacePath().toFile(), folder);
            TransferTask task = new TransferTask();
            task.setSourcePath(repoFolderPath.toString());
            task.setDestPath(destDir.getPath());
